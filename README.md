...
Per usare la modalità Q&A in Pi, avvia Pi dalla radice del repository e usa `/ask-secretary <domanda>`. Le risposte sono bozze: verifica che ogni passaggio citato supporti davvero la frase. I test locali si eseguono con `PYTHONPATH=src python3 -m unittest discover -s tests -v`; metriche e limiti della suite sono in [docs/EVALS.md](docs/EVALS.md).
